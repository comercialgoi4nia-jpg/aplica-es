...
<style>
    ...
    .main-header {
        ...
        color: white;
        box-shadow: 0 8px 32px rgba(13,43,107,0.25);
    }
    .main-header h1 {
        font-size: 2rem;
        font-weight: 700;
    ...
    .stDownloadButton > button {
        width: 100%;
        background: linear-gradient(135deg, #0d2b6b, #2176ff) !important;
        ...
        box-shadow: 0 4px 12px rgba(33,118,255,0.3) !important;
    }
...
def preparar_dados(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filtro obrigatório de Situação = 'Atribuida'
    ...
    # Valor Faturas → numérico
    # Lida com formato BR (ponto = milhar, vírgula = decimal): ex "4.171,84" → 4171.84
    # Também lida com formato já em ponto flutuante: ex "4171.84" → 4171.84
    if 'Valor Faturas' in df.columns:
        def _parse_valor(v):
            ...
            if not s or s in ('nan', 'None', '-'):
                return 0.0
            # Formato BR: contém vírgula → remove pontos de milhar, troca vírgula por ponto
            if ',' in s:
                s = s.replace('.', '').replace(',', '.')
            # Formato já com ponto decimal (sem vírgula): usa direto
            try:
                return float(s)
            except ValueError:
                return 0.0
...
COLUNAS_SAIDA = [
    ...
    'Instalação CCS', 'Situação', 'Valor Faturas', 'Quantidade Faturas'
]

SUBTIPOS_EXCLUIR_ARRECADADO = [
    "P2 VISTORIA - GRUPO A",
    "P1 SUSPENSÃO - GRUPO A",
    "P3 SUSPENSÃO - GRUPO A",
    "P2 SUSPENSÃO - GRUPO A",
    "P3 VISTORIA - GRUPO A",
    "P1 VISTORIA - GRUPO A",
]
...
def modalidade_arrecadado(df, limite_faturas, limite_prefixo):
    ...
    if 'Prefixo' in result.columns:
        result = result.groupby('Prefixo', group_keys=False).head(limite_prefixo)
...
def modalidade_suspensao_p1(df, limite_prefixo):
    ...
    if 'Data Inclusão' in result.columns:
        sort_cols.append('Data Inclusão')
        ascendings.append(True)
    if 'Valor Faturas' in result.columns:
        sort_cols.append('Valor Faturas')
        ascendings.append(False)
    if sort_cols:
        result = result.sort_values(sort_cols, ascending=ascendings)
    if 'Prefixo' in result.columns:
        result = result.groupby('Prefixo', group_keys=False).head(limite_prefixo)
    return selecionar_colunas(result)
...
def gerar_excel(df: pd.DataFrame, modalidade: str) -> bytes:
    wb = Workbook()
    ws = wb.active
    ws.title = modalidade[:30]

    # Paleta
    COR_HEADER_BG   = "0D2B6B"
    ...
    for row_idx, row in enumerate(df.itertuples(index=False), start=2):
        ...
        for col_idx, (col_name, value) in enumerate(zip(headers, row), start=1):
            ...
            if col_name == 'Data Inclusão' and pd.notna(value):
                try:
                    cell_val = pd.Timestamp(value).to_pydatetime()
                except Exception:
                    cell_val = value
            else:
                cell_val = value
            ...
            if col_name in ('Valor Faturas',):
                cell.alignment = Alignment(horizontal="right", vertical="center")
                cell.number_format = 'R$ #,##0.00'
            elif col_name in ('Quantidade Faturas',):
...
with st.sidebar:
    ...
    modalidade = st.selectbox(
        "Modalidade de Prioridade",
        options=[
            "ARRECADADO",
            "QUANTIDADE DE FATURAS",
            "SUSPENSÃO E VISTORIA P1",
            "GRUPO A",
        ],
    ...
    limite_prefixo = st.number_input(
        "Máx. de serviços por Prefixo",
        min_value=1, max_value=100, value=5, step=1,
        help="Limita a quantidade de registros por grupo de Prefixo."
    )

    # Parâmetros dinâmicos por modalidade
    limite_faturas = None
    piso_faturas   = None
    ...
    if modalidade == "ARRECADADO":
        ...
        limite_faturas = st.number_input(
            "Limite máximo de Qtd. Faturas",
            min_value=1, max_value=99999, value=10, step=1,
            help="Filtra apenas registros com Quantidade Faturas ≤ este valor."
        )
        ...
        teto_faturas = st.number_input(
            "Teto máximo de Qtd. Faturas",
            min_value=1, max_value=99999, value=10, step=1,
            help="Filtra apenas registros com Quantidade Faturas ≤ este valor."
        )
...
if arquivo is None:
    ...
    try:
        ...
        with col2:
            st.markdown(f"""
            <div class="metric-card">
                <div class="label">Registros "Atribuida"</div>
        ...
        with col3:
            ...
            st.markdown(f"""
            <div class="metric-card">
                <div class="label">Taxa de Aproveitamento</div>
        ...
        with col4:
            ...
            st.markdown(f"""
            <div class="metric-card">
                <div class="label">Prefixos Distintos</div>
                <div class="value">{prefixos}</div>
            </div>""", unsafe_allow_html=True)
        ...
        <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:0.75rem;">
            ...
            <span style="color:#64748b; font-size:0.9rem;">
                {len(df_result):,} registros encontrados
            </span>
        </div>
        """, unsafe_allow_html=True)
