...
# ─── Lista fixa de equipes do sistema ─────────────────────────────────────────

def gerar_lista_equipes():
    equipes = []
    equipes += [f"GOOC{str(i).zfill(3)}M" for i in range(1, 7)]    # 6 equipes
    equipes += [f"GOOK{str(i).zfill(3)}M" for i in range(1, 13)]   # 12 equipes
    equipes += [f"GOOH{str(i).zfill(3)}M" for i in range(1, 19)]   # 18 equipes
    equipes += [f"GOOL{str(i).zfill(3)}M" for i in range(1, 26)]   # 25 equipes
    return equipes
...
HEADERS_HISTORICO = ["Mês", "Semana", "Prefixo", "Turnos no Mês", "Intervalos na Semana",
                     "Intervalos no Mês"]
HEADERS_STATUS = ["Data", "Prefixo", "Status"]
HEADERS_DESMOBILIZADAS = ["Prefixo", "Data Marcada"]
...
# ─── Google Sheets: Status diário / Desmobilizadas ────────────────────────────

def carregar_desmobilizadas(client, spreadsheet_id):
    ws = get_or_create_sheet(client, spreadsheet_id, "Desmobilizadas")
    if ws is None:
        return pd.DataFrame(columns=HEADERS_DESMOBILIZADAS)
    ensure_headers(ws, HEADERS_DESMOBILIZADAS)
    return load_sheet_df(ws)


def salvar_desmobilizadas(client, spreadsheet_id, df):
    ws = get_or_create_sheet(client, spreadsheet_id, "Desmobilizadas")
    if ws is None:
        return False
    ws.clear()
    dados = [HEADERS_DESMOBILIZADAS]
    for _, r in df.iterrows():
        dados.append([str(r.get(h, "")) for h in HEADERS_DESMOBILIZADAS])
    ws.append_rows(dados)
    return True


def carregar_status_diario(client, spreadsheet_id):
    ws = get_or_create_sheet(client, spreadsheet_id, "Status Diário")
    if ws is None:
        return pd.DataFrame(columns=HEADERS_STATUS)
    ensure_headers(ws, HEADERS_STATUS)
    return load_sheet_df(ws)


def salvar_status_diario(client, spreadsheet_id, df):
    ws = get_or_create_sheet(client, spreadsheet_id, "Status Diário")
    if ws is None:
        return False
    ws.clear()
    dados = [HEADERS_STATUS]
    for _, r in df.iterrows():
        dados.append([str(r.get(h, "")) for h in HEADERS_STATUS])
    ws.append_rows(dados)
    return True
...
def pagina_edicao():
    ...
    with tab_fechar:
        ...
        if df_pendentes.empty:
            ...
            row = df.loc[idx_sel]
            # Identificador estável (não depende da hora atual) para as keys dos widgets abaixo,
            # evitando que o Streamlit "esqueça" o valor digitado a cada rerun.
            widget_id = f"{row['Prefixo']}_{row['Data']}_{idx_sel}"

            with st.form("form_fechar_intervalo"):
                st.markdown(f"### Atualizando: **{row['Prefixo']}**")
                c1, c2 = st.columns(2)
                fim_intervalo = c1.text_input(
                    "🕐 Definir Fim de Intervalo (HH:MM:SS)",
                    value=datetime.now().strftime("%H:%M:%S"),
                    key=f"fim_intervalo_{widget_id}",
                )
                motivo = c2.text_input(
                    "📌 Motivo",
                    value=str(row.get("Motivo", "")),
                    key=f"motivo_fim_{widget_id}",
                )
                ocorrencia = st.text_area(
                    "📋 Ocorrência",
                    value=str(row.get("Ocorrência", "")),
                    height=80,
                    key=f"ocorrencia_{widget_id}",
                )
            ...
            if submitted_fim:
                ...
                try:
                    ...
                    st.success(f"✅ Fim de intervalo registrado às {fim_intervalo} com sucesso!")
                    st.rerun()
                except Exception as e:
    ...
    with tab_atribuir:
        ...
        if df_sem_intervalo.empty:
            ...
            widget_id_atr = f"{row_atr['Prefixo']}_{row_atr['Data']}_{idx_atribuir}"

            with st.form("form_atribuir_intervalo"):
                st.markdown(f"### Atribuir Início para: **{row_atr['Prefixo']}**")
                c1, c2 = st.columns(2)
                inicio_intervalo = c1.text_input(
                    "⏳ Definir Horário de Início (HH:MM:SS)",
                    value=datetime.now().strftime("%H:%M:%S"),
                    key=f"inicio_intervalo_{widget_id_atr}",
                )
                motivo_atr = c2.text_input(
                    "📌 Motivo / Justificativa",
                    value="",
                    key=f"motivo_inicio_{widget_id_atr}",
                )
...
def pagina_confirmacao():
    st.title("🔍 Confirmação de Equipes sem Turno Aberto")
    st.markdown(
        "Para as equipes que **não apareceram na base de hoje**, confirme o status: "
        "Ausente, Atrasada ou Desmobilizada. Equipes marcadas como **Desmobilizada** "
        "deixam de ser perguntadas até reaparecerem na base."
    )
    ...
    ws_turnos = get_or_create_sheet(client, sid, "Turnos")
    df_turnos = load_sheet_df(ws_turnos) if ws_turnos is not None else pd.DataFrame()

    # Equipes que abriram turno (apareceram na base) hoje
    equipes_hoje = set()
    if not df_turnos.empty and "Data" in df_turnos.columns:
        equipes_hoje = set(df_turnos[df_turnos["Data"].astype(str) == hoje_str]["Prefixo"].astype(str))

    df_desmob = carregar_desmobilizadas(client, sid)
    prefixos_desmob = set(df_desmob["Prefixo"].astype(str)) if not df_desmob.empty else set()

    # Se uma equipe desmobilizada reaparece na base de hoje, ela sai da lista de desmobilizadas
    # e volta a ser monitorada normalmente (a pergunta volta a valer para ela).
    reapareceram = prefixos_desmob & equipes_hoje
    if reapareceram:
        df_desmob = df_desmob[~df_desmob["Prefixo"].astype(str).isin(reapareceram)]
        salvar_desmobilizadas(client, sid, df_desmob)
        prefixos_desmob -= reapareceram
        st.info(f"✅ Reapareceram na base e voltaram a ser monitoradas: {', '.join(sorted(reapareceram))}")

    df_status = carregar_status_diario(client, sid)
    ja_respondido_hoje = set()
    if not df_status.empty and "Data" in df_status.columns:
        ja_respondido_hoje = set(df_status[df_status["Data"].astype(str) == hoje_str]["Prefixo"].astype(str))

    lista_completa = gerar_lista_equipes()

    # Precisam de confirmação: não apareceram hoje, não estão desmobilizadas e ainda não foram respondidas hoje
    pendentes = [
        p for p in lista_completa
        if p not in equipes_hoje and p not in prefixos_desmob and p not in ja_respondido_hoje
    ]

    c1, c2, c3 = st.columns(3)
    c1.metric("Total de Equipes", len(lista_completa))
    c2.metric("✅ Abriram Turno Hoje", len(equipes_hoje & set(lista_completa)))
    c3.metric("⏳ Pendentes de Confirmação", len(pendentes))

    if prefixos_desmob:
        with st.expander(f"🚫 Equipes Desmobilizadas ({len(prefixos_desmob)}) — não perguntadas até reaparecerem na base"):
            st.write(", ".join(sorted(prefixos_desmob)))

    st.markdown("---")

    if not pendentes:
        st.success("🎉 Todas as equipes já foram contabilizadas hoje (abriram turno, foram confirmadas ou estão desmobilizadas).")
    else:
        st.warning(f"⚠️ {len(pendentes)} equipe(s) não apareceram na base de hoje e precisam de confirmação:")

        with st.form("form_confirmacao_equipes"):
            respostas = {}
            for prefixo in pendentes:
                respostas[prefixo] = st.radio(
                    f"**{prefixo}**",
                    ["Ausente", "Atrasada", "Desmobilizada"],
                    horizontal=True,
                    key=f"status_{prefixo}_{hoje_str}",
                )
            enviar = st.form_submit_button("💾 Confirmar Status das Equipes", type="primary")

        if enviar:
            novos_status = []
            novas_desmob = []
            for prefixo, status in respostas.items():
                novos_status.append({"Data": hoje_str, "Prefixo": prefixo, "Status": status})
                if status == "Desmobilizada":
                    novas_desmob.append({"Prefixo": prefixo, "Data Marcada": hoje_str})

            df_status_final = pd.concat([df_status, pd.DataFrame(novos_status)], ignore_index=True)
            salvar_status_diario(client, sid, df_status_final)

            if novas_desmob:
                df_desmob_final = pd.concat([df_desmob, pd.DataFrame(novas_desmob)], ignore_index=True)
                salvar_desmobilizadas(client, sid, df_desmob_final)

            st.success("✅ Status das equipes registrado com sucesso!")
            st.rerun()

    if not df_status.empty:
        st.markdown("---")
        st.subheader("📊 Histórico de Status Registrados")
        st.dataframe(
            df_status.sort_values("Data", ascending=False),
            use_container_width=True,
            hide_index=True,
        )
...
def main():
    with st.sidebar:
        ...
        pagina = st.radio(
            "Navegação",
            ["📋 Turnos do Dia", "✏️ Editar Registros", "🔍 Confirmar Equipes", "📈 Histórico", "⚙️ Configurações"],
        )
        st.markdown("---")
